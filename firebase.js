...
function storedStage(data = {}) {
  ...
  const m = mergedMetrics(data);
  if (receiptText) {
    receiptText.innerHTML = `Mensaje #${m.messageNumber || shortMessageNumber(currentMessageKey)}<br><span style="font-weight:500;color:#6d8192">Guardado ${formatClock(m.createdAt)} · esperando aceptación</span>`;
  }
  if (metricsBox) metricsBox.innerHTML = "";
}
...
function deliveredStage(data = {}) {
  ...
  const acceptance = formatDuration(m.createdAt, m.deliveredAt);
  if (receiptText) {
    receiptText.innerHTML = `Mensaje #${m.messageNumber || shortMessageNumber(currentMessageKey)}<br><span style="font-weight:500;color:#1b7953">Entregado ${formatClock(m.deliveredAt)}${acceptance ? ` · aceptación en ${acceptance}` : ""}</span>`;
  }
