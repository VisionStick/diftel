...
function shortMessageNumber(id = "", item = {}) {
  return item?.messageNumber || id.slice(-6).toUpperCase() || "------";
}
...
function passesStatusFilter(item) {
  ...
  if (value === "all") return true;
  if (value === "delivered") return item?.status === "Entregado";
  if (value === "transit") return item?.status !== "Entregado";
  return true;
}

function sortedEntries() {
  return Object.entries(latestMessages || {}).sort((a, b) => {
...
function renderMessages() {
  ...
  const delivered = allEntries.filter(([, item]) => item?.status === "Entregado").length;
  ...
  filterCount.textContent = `${entries.length} ${entries.length === 1 ? "visible" : "visibles"}`;

  if (!entries.length) {
    messagesList.innerHTML = '<div class="empty">No hay mensajes que coincidan con los filtros.</div>';
    return;
  }
  ...
  messagesList.innerHTML = entries.map(([id, item]) => {
    ...
    const number = shortMessageNumber(id, item);
    const created = formatDateTime(item?.createdAt);
    const deliveredAt = item?.deliveredAt ? formatDateTime(item.deliveredAt) : null;
    const duration = item?.deliveredAt ? formatDuration(item?.createdAt, item.deliveredAt) : "";

    const timeDetail = status === "Entregado"
      ? `<span class="msg-meta delivery-time">Entregado: ${escapeHtml(deliveredAt || "sin hora")} ${duration ? `· demoró ${escapeHtml(duration)}` : ""}</span>`
      : `<span class="msg-meta transit-time">Recibido: ${escapeHtml(created)}</span>`;

    const deliveryButton = status === "Entregado"
      ? '<span class="msg-meta">Mensaje finalizado</span>'
      ...
      <article class="msg">
        <div class="msg-top">
          <div>
            <div class="msg-title-row">
              ...
              <span class="message-number">#${escapeHtml(number)}</span>
            </div>
            <div class="msg-meta">${escapeHtml(item?.protocol || "N/A")} · ${escapeHtml(created)} · ID ${escapeHtml(id)}</div>
          </div>
        ...
        <div class="msg-actions">
          ...
          <div class="action-buttons">
            ${deliveryButton}
            <button class="btn danger delete-btn" data-id="${escapeHtml(id)}" data-number="${escapeHtml(number)}">Eliminar</button>
          </div>
        </div>
      </article>`;
  }).join("");
  ...
  document.querySelectorAll(".delete-btn").forEach(button => {
    button.addEventListener("click", async () => {
      ...
      const confirmed = window.confirm(`¿Eliminar definitivamente el mensaje #${number}? Esta acción no se puede deshacer.`);
      if (!confirmed) return;
      ...
      try {
        ...
        button.disabled = false;
        button.textContent = "Reintentar";
        alert("No se pudo eliminar. Debes permitir el borrado al administrador en las reglas de Realtime Database.");
      }
    });
  });
}
...
loginForm.addEventListener("submit", async event => {
  ...
  loginFeedback.style.color = '#167fa3';
...
timeFilter?.addEventListener("change", renderMessages);
